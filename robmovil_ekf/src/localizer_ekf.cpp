...
void robmovil_ekf::LocalizerEKF::makeA(void)
{
  ...
  A(1,3) = - u(1) *delta_t*sin(x(3));
  A(2,3) =   u(1)*delta_t*cos(x(3));
...
void robmovil_ekf::LocalizerEKF::makeH(void)
{
  ...
  if (diff_robot_landmark.length2() < 0.001)
  {
    ...
    auto rho = robot_position.distance(correspondence_landmark);

    H(1,1) = (-correspondence_landmark.getX()+robot_position.getX()) / rho;
    H(1,2) = (-correspondence_landmark.getY()+robot_position.getY()) / rho;
    H(2,1) = -H(1,2);
    H(2,2) = H(1,1);
  }
...
void robmovil_ekf::LocalizerEKF::makeProcess(void)
{
  ...
  x(1) = x_old(1) + u(1)*delta_t*cos(x(3));
  x(2) = x_old(2) + u(1)*delta_t*sin(x(3));
...
bool robmovil_ekf::LocalizerEKF::find_corresponding_landmark(const tf::Point& measured_landmark, tf::Point& corresponding_landmark, float delta_radio)
{
  ...
  {
    tf::Point &landmark = map_landmarks[i];
    /* COMPLETAR */
    tfScalar distance = measured_landmark.distance(landmark);
    if (distance < delta_radio && distance < min_distance)
    {
        corresponding_landmark = landmark;
...
robmovil_ekf::LocalizerEKF::Vector robmovil_ekf::LocalizerEKF::landmark2measure(const tf::Point& landmark)
{
  ...
  measure(2) = atan2( relative_landmark.getY(), relative_landmark.getX() ); // Calculo del phi
